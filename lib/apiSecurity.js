...
function requireBearerTokenMatch(req, res, options) {
  ...
  const validTokens = (allowedTokens || [expectedToken]).filter(Boolean)
  ...
  if (!validTokens.includes(bearerToken.token)) {
    res.status(403).json({
...
export function requireCronOrOperationalRouteAccess(req, res) {
  return requireBearerTokenMatch(req, res, {
    allowedMethods: ["GET", "POST"],
    allowedTokens: [getCronSecret(), getOperationalRouteSecret()],
    missingSecretError: "CRON_SECRET or admin API secret is not configured",
    missingAuthErrorMessage: "Missing Authorization header"
  })
}

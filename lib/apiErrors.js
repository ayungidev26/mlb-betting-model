const PUBLIC_ERROR_DEFINITIONS = [
  ...
  {
    match: (error) => error?.code === "UPSTREAM_DATA_INCOMPLETE",
    statusCode: 503,
    code: "UPSTREAM_DATA_INCOMPLETE",
    message: "Upstream data is temporarily unavailable"
  },
  {
    match: (error) => typeof error?.message === "string" && error.message.startsWith("Team ratings stale:"),
    statusCode: 503,
    code: "RATINGS_REFRESH_REQUIRED",
    message: "Team ratings must be refreshed before model execution"
  },
  {
    match: (error) => error?.code === "INCOMPLETE_PREDICTION_BATCH",
    statusCode: 503,
    code: "INCOMPLETE_PREDICTION_BATCH",
    message: "One or more game predictions could not be generated",
    details: (error) => ({
      eligibleGames: error?.diagnostics?.eligibleGames ?? null,
      predictionsSucceeded: error?.diagnostics?.predictionsSucceeded ?? null,
      predictionsFailed: error?.diagnostics?.predictionsFailed ?? null,
      failedGameIds: Array.isArray(error?.diagnostics?.failedGameIds)
        ? error.diagnostics.failedGameIds
        : []
    })
  },
  {
  ...
  },
  {
    match: (error) => {
      ...
      return message.startsWith("Games cache missing") ||
        (error?.name === "ValidationError" && message.startsWith("Games payload")) ||
        (message.startsWith("Games ") && message.includes("Refresh the upstream cache"))
    },
    statusCode: 503,
    code: "GAMES_CACHE_INVALID",
    message: "Games cache must be refreshed before model execution"
  },
  {
    match: (error) => {
      const message = typeof error?.message === "string" ? error.message : ""
      return message.startsWith("mlb:stats:") && message.includes("Refresh the upstream cache")
    },
    statusCode: 503,
    code: "STATS_CACHE_INVALID",
    message: "Stats cache must be refreshed before model execution"
  },
  {
    match: (error) => {
      const message = typeof error?.message === "string" ? error.message : ""
      return message.startsWith("Odds ") && message.includes("Refresh the upstream cache")
    },
    statusCode: 503,
    code: "ODDS_CACHE_INVALID",
    message: "Odds cache must be refreshed before edge generation"
  },
  {
...
export function sendRouteError(res, route, error, context = {}) {
  ...
  const details = typeof publicError?.details === "function"
    ? publicError.details(error)
    : undefined

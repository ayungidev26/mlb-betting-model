...
export function normalizeBullpenStatRecord(
  ...
  const fallbackRecord = fallbackStat ? readBullpenStatValue(fallbackStat, advancedStat || {}) : null
  const fip = primaryRecord.fip ?? fallbackRecord?.fip
  const xfip = primaryRecord.xfip ?? fallbackRecord?.xfip

  return {
    ...primaryRecord,
    fip: fip ?? calculateFallbackFip(stat, leagueContext, fallbackStat),
    xfip: xfip ?? calculateFallbackXFip(stat, leagueContext, fallbackStat),
    strikeoutRate: primaryRecord.strikeoutRate ?? fallbackRecord?.strikeoutRate ?? null,
    walkRate: primaryRecord.walkRate ?? fallbackRecord?.walkRate ?? null,
    strikeoutMinusWalkRate: (
      primaryRecord.strikeoutMinusWalkRate ??
      fallbackRecord?.strikeoutMinusWalkRate ??
      calculateStrikeoutMinusWalkRate(
        primaryRecord.strikeoutRate ?? fallbackRecord?.strikeoutRate ?? null,
        primaryRecord.walkRate ?? fallbackRecord?.walkRate ?? null
      )
    ),
    homeRunsPer9: primaryRecord.homeRunsPer9 ?? fallbackRecord?.homeRunsPer9 ?? null,
    battingAverageAgainst: primaryRecord.battingAverageAgainst ?? fallbackRecord?.battingAverageAgainst ?? null,
    leftOnBaseRate: primaryRecord.leftOnBaseRate ?? fallbackRecord?.leftOnBaseRate ?? null,
...
function calculateFallbackFip(stat, leagueContext, fallbackStat = null) {
  const source = stat || fallbackStat
  if (!source) return null

  const homeRuns = parseNumericValue(source?.homeRuns)
  const walks = parseNumericValue(source?.baseOnBalls)
  const hitByPitch = parseNumericValue(source?.hitBatsmen) || 0
  const strikeouts = parseNumericValue(source?.strikeOuts)
  const inningsPitched = parseInningsPitched(source?.inningsPitched)
...
function calculateFallbackXFip(stat, leagueContext, fallbackStat = null) {
  const source = stat || fallbackStat
  if (!source) return null
  ...
  const strikeouts = parseNumericValue(source?.strikeOuts)
  const inningsPitched = parseInningsPitched(source?.inningsPitched)
  const homeRuns = parseNumericValue(source?.homeRuns) || 0
  const flyOuts = parseNumericValue(source?.flyOuts)
...
export async function fetchBullpenStatsByTeam(fetchContext = {}) {
  ...
  const fallbackStatsByTeamId = new Map()

  for (const team of teams) {
    const reliefStats = readPrimaryStat(await fetchJsonWithRetry(buildStatUrl(team.id, "sitCodes=rp")))
    const fallbackStats = readPrimaryStat(await fetchJsonWithRetry(buildStatUrl(team.id)))
    ...
    if (fallbackStats) {
      fallbackStatsByTeamId.set(team.id, fallbackStats)
      if (!reliefStats) {
        allTeamPitchingStats.push(fallbackStats)
      }
    }
  }
  ...
  for (const team of teams) {
    ...
    const fallbackStat = fallbackStatsByTeamId.get(team.id) || null

    if (!reliefStat && !fallbackStat) {
      continue
    }

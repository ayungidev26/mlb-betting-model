...
export function isRelieverStat(stat) {
  const gamesInRelief = parseNumericValue(stat?.gamesInRelief) || 0
  ...
  const reliefAppearances = parseNumericValue(stat?.reliefAppearances) || 0

  const inferredReliefAppearances = Math.max(gamesPitched - gamesStarted, 0)
  const totalReliefWork = Math.max(gamesInRelief, reliefAppearances, inferredReliefAppearances)

  // Explicit reliever definition:
  // - include pitchers with at least one relief appearance
  // - exclude pitchers used only as starters
  // - include swingmen with both starts and relief appearances
  return gamesPitched > 0 && (totalReliefWork > 0 || saves > 0 || holds > 0)
}
...
function summarizeRelieverSample(relievers = [], max = 5) {
  return relievers
    .slice(0, max)
    .map((reliever) => `${reliever.name} (${reliever.playerId})`)
}

export function dedupePitchersById(pitchers = []) {
  ...
  for (const pitcher of pitchers) {
    ...
    if (seenIds.has(pitcher.playerId)) {
      continue
    }

    seenIds.add(pitcher.playerId)
    uniquePitchers.push(pitcher)
  }

  return uniquePitchers
}

async function buildTeamRelieverPool(team) {
  const roster = await fetchTeamPitcherRoster(team.id)
  const reviewedPitchers = []
  const nonPitcherEntries = []

  for (const rosterEntry of roster) {
    if (rosterEntry?.position?.abbreviation !== "P") {
      nonPitcherEntries.push(rosterEntry)
      continue
    }
    ...
    reviewedPitchers.push({
      playerId: String(playerId),
      name: rosterEntry?.person?.fullName || `Player ${playerId}`,
      seasonStat
    })
  }

  const dedupedPitchers = dedupePitchersById(reviewedPitchers)
  const relievers = dedupedPitchers.filter((pitcher) => isRelieverStat(pitcher.seasonStat))

  const stats = {
    teamId: team.id,
    teamName: team.name,
    rosterEntries: roster.length,
    nonPitcherEntries: nonPitcherEntries.length,
    reviewedPitchers: reviewedPitchers.length,
    dedupedPitchers: dedupedPitchers.length,
    duplicatePitchers: Math.max(reviewedPitchers.length - dedupedPitchers.length, 0),
    qualifiedRelievers: relievers.length,
    relieverSample: summarizeRelieverSample(relievers)
  }

  console.info(
    "fetchBullpenStats: reliever filter summary",
    JSON.stringify(stats)
  )

  return { relievers, stats }
}
...
async function buildBullpenAdvancedStatMap(teams, savantPitcherStats, relieverPoolByTeamId = new Map()) {
  const advancedStatsByTeam = {}

  for (const team of teams) {
    try {
      const relieverPool = relieverPoolByTeamId.get(team.id) || { relievers: [] }
      const relievers = relieverPool.relievers || []
      const weightedMetrics = {
        hardHitRate: { weightedTotal: 0, weight: 0 },
        barrelRate: { weightedTotal: 0, weight: 0 },
        averageExitVelocity: { weightedTotal: 0, weight: 0 }
      }
      const relieverUsagePool = []

      for (const reliever of relievers) {
        const inningsPitched = parseInningsPitched(reliever?.seasonStat?.inningsPitched)
        const battersFaced = parseBattersFaced(reliever?.seasonStat)
        const weight = battersFaced || inningsPitched || 0
        const advancedStat = savantPitcherStats
          ? getAdvancedPitcherStats(
            reliever.playerId,
            reliever.name,
            savantPitcherStats
        ...
        relieverUsagePool.push({
          playerId: String(reliever.playerId),
          inningsPitched: inningsPitched || 0,
          battersFaced: battersFaced || 0
        })
      }

      const keyRelieverIds = relieverUsagePool
      ...
      if (relievers.length < 3) {
        console.warn(
          `fetchBullpenStats: limited reliever pool for ${team.name}`,
          JSON.stringify({
            qualifiedRelievers: relievers.length,
            keyRelieverIds
          })
...
export async function fetchBullpenStatsByTeam(fetchContext = {}) {
  ...
  const relieverPoolByTeamId = new Map()

  for (const team of teams) {
    try {
      const relieverPool = await buildTeamRelieverPool(team)
      relieverPoolByTeamId.set(team.id, relieverPool)
    } catch (error) {
      console.warn(
        `fetchBullpenStats: unable to evaluate reliever pool for ${team?.name || team?.id}`,
        error?.message || error
      )
      relieverPoolByTeamId.set(team.id, {
        relievers: [],
        stats: {
          teamId: team.id,
          teamName: team.name,
          reviewedPitchers: 0,
          qualifiedRelievers: 0,
          relieverSample: []
        }
      })
    }
  ...
  try {
    advancedStatsByTeam = await buildBullpenAdvancedStatMap(teams, savantPitcherStats, relieverPoolByTeamId)
  } catch (error) {
  ...
  for (const team of teams) {
    ...
    if (!relieverPool.relievers.length) {
      console.warn(`fetchBullpenStats: skipping bullpen storage for ${team.name} due to no qualifying relievers`)
      continue
    }

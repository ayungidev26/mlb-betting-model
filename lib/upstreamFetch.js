...
async function fetchWithRetry(url, options = {}, parseResponse) {
  const {
    fetchImpl = fetch,
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      return await parseResponse(response, contentType)
    } catch (error) {
      lastError = error
...
export async function fetchTextWithRetry(url, options = {}) {
  return fetchWithRetry(url, {
    ...options,
    accept: options.accept || "text/plain, text/csv;q=0.9, */*;q=0.8"

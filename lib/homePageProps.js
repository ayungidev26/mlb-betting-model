  ...
  return games.map((game) => {
    ...
    return matchedEdge
      ? {
          ...
          ...(game.lifecycle === "completed" ? { marketStatus: "historical" } : {})
        }
      : {
          ...game,
          edge: null,
...
function sortGamesByEdge(games = []) {
  return [...games].sort((leftGame, rightGame) => {
    const lifecycleOrder = { live: 0, upcoming: 1, completed: 2 }
    const lifecycleDifference =
      (lifecycleOrder[leftGame?.lifecycle] ?? 1) -
      (lifecycleOrder[rightGame?.lifecycle] ?? 1)
...
export function classifyDashboardFreshness({ predictionMetadata, edgeMetadata, oddsMetadata, expectedDateKey = getEasternDateKey() } = {}) {
  const metadata = [predictionMetadata, oddsMetadata, edgeMetadata]
  if (metadata.every(item => !item)) return { status: "healthy", message: "Showing cached predictions (legacy metadata)." }
  if (metadata.some(item => !item)) return { status: "unavailable", message: "Current slate metadata is unavailable; recommendations are hidden." }
  if (metadata.some(item => item.status && item.status !== "healthy")) return { status: "degraded", message: "A pipeline refresh is degraded; recommendations are hidden." }
  if (metadata.some(item => item.dateKey !== expectedDateKey)) return { status: "stale", message: "Cached recommendations are stale; current recommendations are hidden." }
  if (edgeMetadata.predictionGeneratedAt !== predictionMetadata.generatedAt || edgeMetadata.oddsGeneratedAt !== (oddsMetadata.generatedAt || oddsMetadata.fetchedAt)) {
    return { status: "stale", message: "Pipeline inputs do not share the same lineage; recommendations are hidden." }
  }
  return { status: "healthy", message: "Showing current verified predictions." }
}

export function buildHomePageViewModel({ predictions, edges = [], odds = [], predictionMetadata, edgeMetadata, oddsMetadata } = {}) {
  const cachedPredictions = Array.isArray(predictions)
    ? predictions.filter(Boolean)
    : []
  const freshness = classifyDashboardFreshness({ predictionMetadata, edgeMetadata, oddsMetadata })
  const canShowMarkets = freshness.status === "healthy"
  const games = sortGamesByEdge(mergeGamesWithEdges(cachedPredictions, canShowMarkets ? edges : [], canShowMarkets ? odds : []))
  const recommendedBets = games.filter((game) => (
    game.lifecycle !== "completed" && typeof game.edge === "number"
  )).length

  return {
    games,
    ...(predictionMetadata || edgeMetadata || oddsMetadata ? { freshness } : {}),
    summary: {
      predictionsCreated: cachedPredictions.length,
      recommendedBets,
      message: cachedPredictions.length > 0
        ? (predictionMetadata || edgeMetadata || oddsMetadata ? freshness.message : "Showing cached predictions.")
...
export function buildPublicApiUrl(req, path) {
  const configuredOrigin = process.env.DEPLOYMENT_ORIGIN
  if (!configuredOrigin) throw new Error("DEPLOYMENT_ORIGIN is not configured")
  const origin = new URL(configuredOrigin).origin
  const requestHost = req?.headers?.host
  if (requestHost && requestHost !== new URL(origin).host) {
...
export async function loadHomePageData(req, fetchImpl = fetch) {
  const [predictionPayload, edgePayload, oddsPayload] = await Promise.all([
    loadPublicJson(req, "/api/predictions", fetchImpl),
    loadPublicJson(req, "/api/edges", fetchImpl),
    loadPublicJson(req, "/api/odds", fetchImpl)
  ])
  ...
  return {
    ...
    ...(predictionPayload.metadata ? { predictionMetadata: predictionPayload.metadata } : {}),
    ...(edgePayload.metadata ? { edgeMetadata: edgePayload.metadata } : {}),
    ...(oddsPayload.metadata ? { oddsMetadata: oddsPayload.metadata } : {})
  }
}

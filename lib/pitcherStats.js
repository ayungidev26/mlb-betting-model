...
export const SAVANT_PITCHER_REQUIRED_COLUMNS = [
  "player_id",
  "pitcher",
  "k_percent",
  "bb_percent",
...
export function buildSavantPitcherUrl(season) {
  const selections = [
    "player_id",
    "pitcher",
    "k_percent",
  ...
  return `https://baseballsavant.mlb.com/leaderboard/custom?year=${season}&type=pitcher&filter=&min=0&selections=${encodeURIComponent(selections)}&chart=false&x=xera&y=xera&r=no&chartType=beeswarm&csv=true`
}
...
export function asNumberOrNull(value, places = 3) {
  const parsed = parseNumericValue(value)
...
export async function fetchSavantPitcherStatMap(season, fetchImpl = fetch) {
  const savantUrl = buildSavantPitcherUrl(season)
...
export function normalizePitcherStatRecord(stat, advancedStat = null, leagueContext = null) {
  ...
  const record = {
    wins: parseNumericValue(stat?.wins),
    losses: parseNumericValue(stat?.losses),
